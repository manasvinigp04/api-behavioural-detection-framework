...
class RulesEngine:
    ...
    def _validate_rule(
        ...
        # FIXED: Skip validation for fields inside empty arrays
        # Example: "response.200.resources[].name" when resources[].length == 0
        if "[]" in rule.field_path:
            # Get the array field name (part with [])
            # e.g., "response.200.resources[].name" -> find "resources[]"
            parts = rule.field_path.split(".")
            for part in parts:
                if part.endswith("[]"):
                    # Found array field, extract its value
                    array_field_path = ".".join(parts[:parts.index(part)+1])
                    # Get just the field name without []
                    array_field = part[:-2]

                    # Navigate to the array
                    array_path_parts = array_field_path.split(".")
                    if array_path_parts[0] in ["request", "response", "parameter"]:
                        array_path_parts = array_path_parts[1:]
                    if array_path_parts and array_path_parts[0].isdigit():
                        array_path_parts = array_path_parts[1:]

                    # Get the array value
                    current = data
                    for p in array_path_parts[:-1]:  # Navigate to parent
                        if isinstance(current, dict):
                            current = current.get(p)

                    if isinstance(current, dict):
                        array_value = current.get(array_field)
                        # If array exists but is empty, skip validation of its items
                        if isinstance(array_value, list) and len(array_value) == 0:
                            # Empty array is valid - can't validate schema of non-existent items
                            return violations
                    break
    ...
    def _get_field_value(self, data: Dict[str, Any], field_path: str) -> Any:
        ...
        for part in path_parts:
            ...
            # FIXED: Handle array indicators like "resources[]"
            # Remove [] suffix if present
            if part.endswith("[]"):
                array_field = part[:-2]  # Remove the "[]" suffix
                ...
                if isinstance(current, dict):
                    ...
                    # If it's an array, return the first item for validation
                    # (we validate array items individually elsewhere)
                    if isinstance(current, list):
                        if len(current) > 0:
                            current = current[0]
                        else:
                            # Empty array - can't extract field value from empty array
                            return None
                else:
                    return None
            else:
                # Regular field access (no array indicator)
                if isinstance(current, dict):
